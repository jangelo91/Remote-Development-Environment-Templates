...
public class Maze {
	...
	// array of booleans where true means there is a wall there, false means there
	// isn't
	...
	public Maze() {
		new Maze(30);
		// no argument constructor automatically sets the density to the default value
		// of 30
	}

	public Maze(int density) {
		walls = generateMaze(density);
		// create a randomly filled grid of walls, where the higher the density, the
		// more walls there will be
	...
	public boolean isWallLeft(int x, int y) {
		// inputs: x: x coordinate of the grid square being checked, y: y coordinate of
		// the grid square being checked
	...
	private Square[] neighbours(Square square) {
		// returns all squares adjacent to and in the same area as the square inputed,
		// including itself
		...
		if (!isWallLeft(square.getxCoord(), square.getyCoord())) {
			neighbours[numberOfNeighbours] = new Square(square.getxCoord() - 1, square.getyCoord());
			numberOfNeighbours++;
		}
		...
		if (!isWallAbove(square.getxCoord(), square.getyCoord())) {
			neighbours[numberOfNeighbours] = new Square(square.getxCoord(), square.getyCoord() - 1);
			numberOfNeighbours++;
		}
	...
	private static boolean isNeighbour(Square a, Square b) {
		...
		int x2 = b.getxCoord();
		int y2 = b.getyCoord();
		return (x2 - 1 == x && y2 == y) || (x2 + 1 == x && y2 == y) || (x2 == x && y2 - 1 == y)
				|| (x2 == x && y2 + 1 == y);
	}

	private void removeWall(Square[] area) {
		// when given an array of squares that make up an area, remove a random wall (or
		// two) between that area and another
	...
	private Square[] areaNeighbours(Square[] area) {
		// when given an array of squares that make up an area, returns all neighbours
		// of that area
	...
	private Square[] walledNeighbours(Square square) {
		// returns all neighbour squares of the input square which are separated by a
		// wall
		...
		for (int i = 0; i < list.size() - 1; i++) {
			for (int j = (i + 1); j < list.size(); j++) {
				if (intersection(list.get(i), list.get(j)).length != 0) {
					...
					i = 0;
					j = 0;
				} // do this for 1;2, 1;3; 1;4, 1;5, 1;6... 2;3, 2;4, 2;5, ... n-1;n , resetting
					// each time a merge is successful
			}
		}
		// merge all the areas with squares in common}
	...
	public String toString() {
		...
		for (int i = 1; i < 8; i++) {
			lines[i] = "";
			for (int j = 0; j < 8; j++) {
				lines[i] = lines[i] + (walls[1][j][i - 1] ? "|" : " ") + (walls[0][i][j] ? "_" : " ");
			}
		}
		String toReturn = "";
		for (int i = 0; i < 8; i++) {
