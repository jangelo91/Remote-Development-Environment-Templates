public class Player {
	...
	private void move(double speed) {
		Point nextPoint = new Point(
				this.coordinates.getxCoord() + (speed * Math.cos(Math.toRadians(90 - this.direction))),
				this.coordinates.getyCoord() - (speed * Math.sin(Math.toRadians(90 - this.direction))));
		if (cornerCrash(nextPoint, GameEngine.tankWidth)) {// if the tank would crash into a corner...
			nextPoint = new Point(this.coordinates.getxCoord()// try moving it horizontally
					+ (speed * Math.cos(Math.toRadians(90 - this.direction))), this.coordinates.getyCoord());
			if (cornerCrash(nextPoint, GameEngine.tankWidth) || wallCrashVertical(nextPoint, GameEngine.tankWidth)
				...
				nextPoint = new Point(this.coordinates.getxCoord(), // if moving it horizontally would cause a crash,
																	// try moving it vertically
						this.coordinates.getyCoord() - (speed * Math.sin(Math.toRadians(90 - this.direction))));
				if (cornerCrash(nextPoint, GameEngine.tankWidth) || wallCrashVertical(nextPoint, GameEngine.tankWidth)
						|| wallCrashHorizontal(nextPoint, GameEngine.tankWidth)) {
					nextPoint = new Point(this.coordinates.getxCoord(), // if moving it vertically would also cause a
																		// crash, just don't move it
							this.coordinates.getyCoord());// TODO stop it going through T walls
				}
			}
		} else {
			if (wallCrashVertical(nextPoint, GameEngine.tankWidth)
					&& wallCrashHorizontal(nextPoint, GameEngine.tankWidth)) {
				nextPoint = new Point(this.coordinates.getxCoord(), // if moving it at all would cause a crash, just
																	// don't move it
	...
	private boolean wallCrashVertical(Point p, int w) {
		// when given a point p, returns whether or not the circular object of width w
		// at that point is in/over a wall
		w = w / 2;
		int xWall = 0;
		...
		boolean inRightWall = (p.getxCoord() + w >= ((int) this.currentXSquare() + 1 + xWall) * GameEngine.squareWidth
	...
	private boolean wallCrashHorizontal(Point p, int w) {
		...
		boolean inTopWall = (p.getyCoord() - w <= ((int) this.currentYSquare()) * GameEngine.squareWidth
		...
		boolean inBottomWall = (p.getyCoord() + w >= ((int) this.currentYSquare() + 1 + yWall) * GameEngine.squareWidth
	...
	private boolean cornerCrash(Point p, int w) {
		// when given a point p, returns whether or not the circular object of width w
		// at that point is in/over the nearest corner
		w = w / 2;
		...
		Point p2 = new Point(x * (GameEngine.wallWidth + GameEngine.squareWidth) + GameEngine.wallWidth,
				y * (GameEngine.wallWidth + GameEngine.squareWidth));
	...
	public void shoot() {
		if (this.numberOfBulletsFired < 5) {
			...
			Point bulletStart = new Point(
					this.coordinates.getxCoord() + ((GameEngine.bulletWidth / 2 + GameEngine.tankWidth / 2)
							* Math.cos(Math.toRadians(90 - this.direction))),
					this.coordinates.getyCoord() - ((GameEngine.bulletWidth / 2 + GameEngine.tankWidth / 2)
			...
			if (wallCrashVertical(bulletStart, GameEngine.bulletWidth)
					...
					|| cornerCrash(bulletStart, GameEngine.bulletWidth)) { // if the bullet would start in/over any of
																			// the walls
				this.hit();
			} else {
	...
	public double currentXSquare() {
		...
		for (int i = 0; i < 7; i++) {
			if ((GameEngine.wallWidth * (i + 1) + GameEngine.squareWidth * i < this.coordinates.getxCoord())
					&& (GameEngine.wallWidth * (i + 1) + GameEngine.squareWidth * (i + 1)) > this.coordinates
							.getxCoord()) {
				return i;
			}
		}
