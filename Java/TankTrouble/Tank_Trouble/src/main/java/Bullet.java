public class Bullet {
	...
	public void moveBullet() {
		Point nextPoint = new Point(
				this.position.getxCoord() + (bulletSpeed * Math.cos(Math.toRadians(90 - this.angle))),
				this.position.getyCoord() - (bulletSpeed * Math.sin(Math.toRadians(90 - this.angle))));
		if (wallCrashHorizontal(nextPoint, GameEngine.bulletWidth)) { // if the bullet would go over any horizontal
																		// walls
			flipBulletH();
			nextPoint = new Point(this.position.getxCoord() + (bulletSpeed * Math.cos(Math.toRadians(90 - this.angle))),
					this.position.getyCoord());
		} else if (wallCrashVertical(nextPoint, GameEngine.bulletWidth)) { // if the bullet would go over any vertical
																			// walls
			flipBulletV();
	...
	public boolean reduceTimer() {
		// count down the timer. If time is up, remove the bullet. Return whether the
		// bullet was removed
		timer--;
		if (timer < 0) {
			this.removeBullet();
	...
	public void tankCollision() {
		// checks whether the bullet has collided with player1 or player2, calling the
		// appropriate functions if so
	...
	public double currentXSquare() {
		...
		for (int i = 0; i < 7; i++) {
			if ((GameEngine.wallWidth * (i + 1) + GameEngine.squareWidth * i < this.position.getxCoord())
					&& (GameEngine.wallWidth * (i + 1) + GameEngine.squareWidth * (i + 1)) > this.position
							.getxCoord()) {
				return i;
			}
		}
	...
	private boolean wallCrashVertical(Point p, int w) {
		...
		// at that point is in/over a vertical wall
		w = w / 2;
		int xWall = 0;
	...
	private boolean wallCrashHorizontal(Point p, int w) {
		// when given a point p, returns whether or not the circular object of width w
		// at that point is in/over a horizontal wall
		w = w / 2;
		int yWall = 0;
		if (this.currentYSquare() != (int) this.currentYSquare()) {
