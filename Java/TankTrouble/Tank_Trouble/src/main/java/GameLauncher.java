...
public class GameLauncher extends Canvas implements Runnable {
	...
	private void init() {
		...
		try {
			...
			tank1 = loader.loadImage("../resources/tank1.png");
			tank2 = loader.loadImage("../resources/tank2.png");
			bullet = loader.loadImage("../resources/bullet.png");
			hWall = loader.loadImage("../resources/hWall.png");
			vWall = loader.loadImage("../resources/vWall.png");
	...
	private void render() {
		...
		for (int x = 0; x < 7; x++) {
			for (int y = 0; y < 7; y++) {
				if (engine.maze.isWallBelow(x, y)) {
					g.drawImage(hWall, (GameEngine.squareWidth + GameEngine.wallWidth) * x,
							(GameEngine.squareWidth + GameEngine.wallWidth) * (y + 1), this);
				}
				if (engine.maze.isWallRight(x, y)) {
					g.drawImage(vWall, (GameEngine.squareWidth + GameEngine.wallWidth) * (x + 1),
							(GameEngine.squareWidth + GameEngine.wallWidth) * y, this);
				}
			}
		}
		...
		AffineTransform tx2 = AffineTransform.getRotateInstance(Math.toRadians(engine.player2.getDirection()),
		...
		for (int i = 0; i < GameEngine.bulletList.size(); i++) {
			g.drawImage(bullet,
					(int) (GameEngine.bulletList.get(i).getPosition().getxCoord() - GameEngine.bulletWidth / 2),
					(int) (GameEngine.bulletList.get(i).getPosition().getyCoord() - GameEngine.bulletWidth / 2), this);
		}
